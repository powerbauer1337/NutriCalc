...
const FertilizerManager = ({
  ...
  <div className="space-y-2 max-h-[400px] lg:max-h-[calc(100vh-350px)] overflow-y-auto p-0.5">
      ...
      return (
        <div key={fert.id} className={`p-2 border rounded-md ${fert.active ? 'bg-white dark:bg-slate-800 border-blue-300 dark:border-blue-600' : 'bg-slate-100 dark:bg-slate-900/50 border-slate-300 dark:border-slate-700'}`}>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <button onClick={() => removeFertilizer(fert.id)} className="text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 p-0.5" title="Entfernen">×</button>
          </div>
          <div className="flex items-center gap-1.5">
            <input
              type="number"
              ...
              className="flex-1 px-2 py-1 border rounded-md text-xs disabled:opacity-60 dark:bg-slate-700 dark:text-slate-100"
              min="0"
    ...
    <div className="border-t border-slate-300 dark:border-slate-700 pt-2 mt-2">
      <select onChange={(e) => addFertilizer(e.target.value)} className="w-full px-2 py-1 border rounded-md text-xs dark:bg-slate-700 dark:text-slate-100" value="">
