...
const MixingAssistant = ({ fertilizerDatabase, GROWTH_STAGES, WATER_TYPES }) => {
  ...
  if (!fertilizerDatabase || !GROWTH_STAGES || !WATER_TYPES) {
    return <div className="max-w-lg mx-auto p-4 bg-white dark:bg-slate-800 rounded-lg shadow">Daten werden geladen...</div>;
  }
  ...
  const [nutrientResults, setNutrientResults] = useState({ nutrients: {}, contributions: {}, stage: GROWTH_STAGES[growthStage] });
  ...
  useEffect(() => {
    const results = calculateNutrientResults({
      ...
      fertilizerDatabase,
      NUTRIENT_FIELDS: [
        { key: 'n' }, { key: 'p' }, { key: 'k' }, { key: 'ec' }, { key: 'ph' }
      ],
      GROWTH_STAGES,
      WATER_TYPES,
      customWaterProfile: {},
  ...
  }, [waterAmount, growthStage, waterType, selectedFertilizers, fertilizerDatabase, GROWTH_STAGES, WATER_TYPES]);
  ...
  const addFertilizer = (id) => {
    ...
    setSelectedFertilizers(prev => [...prev, { id, amount: fert?.type === 'powder' ? 0.1 : 1.0 }]);
  };
  ...
  const mixSummary = selectedFertilizers.map(f => {
    const fert = fertilizerDatabase[f.id];
    return fert ? `${fert.name}: ${f.amount} ${fert.unit || (fert.type === 'powder' ? 'g' : 'ml')}` : '';
  }).filter(Boolean);
  ...
  return (
    <div className="max-w-lg mx-auto p-4 bg-white dark:bg-slate-800 rounded-lg shadow space-y-6">
      <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-4">Misch-Assistent</h2>
      {/* Stepper */}
      <div className="flex items-center gap-2 mb-4">
        <div className={`w-8 h-8 flex items-center justify-center rounded-full ${step === 1 ? 'bg-blue-600 text-white' : 'bg-slate-300 dark:bg-slate-700 text-slate-700 dark:text-slate-200'}`}>1</div>
        <span className="text-sm">Wasser & Phase</span>
        <div className="w-6 h-0.5 bg-slate-300 dark:bg-slate-700 mx-2" />
        <div className={`w-8 h-8 flex items-center justify-center rounded-full ${step === 2 ? 'bg-blue-600 text-white' : 'bg-slate-300 dark:bg-slate-700 text-slate-700 dark:text-slate-200'}`}>2</div>
        <span className="text-sm">Dünger</span>
        <div className="w-6 h-0.5 bg-slate-300 dark:bg-slate-700 mx-2" />
        <div className={`w-8 h-8 flex items-center justify-center rounded-full ${step === 3 ? 'bg-blue-600 text-white' : 'bg-slate-300 dark:bg-slate-700 text-slate-700 dark:text-slate-200'}`}>3</div>
      ...
      {step === 1 && (
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium mb-1" htmlFor="waterAmount">Wassermenge (L)</label>
            <input
              id="waterAmount"
              type="number"
              min="0.1"
          ...
          <div className="flex justify-end">
            <Button onClick={() => setStep(2)} variant="primary">Weiter</Button>
          </div>
        </div>
      )}
      {/* Step 2: Fertilizers */}
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {Object.entries(fertilizerDatabase).map(([id, fert]) => (
                <Button key={id} variant="secondary" onClick={() => addFertilizer(id)} disabled={!!selectedFertilizers.find(f => f.id === id)} className="text-xs mb-1">
                  {fert.name}
                </Button>
              ))}
            </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {selectedFertilizers.length === 0 && <li className="text-slate-500">Noch keine Dünger ausgewählt.</li>}
              {selectedFertilizers.map(f => {
                ...
                return (
                  <li key={f.id} className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-xs">{fert?.unit || (fert?.type === 'powder' ? 'g' : 'ml')}</span>
                    <Button onClick={() => removeFertilizer(f.id)} variant="danger" className="text-xs">Entfernen</Button>
                  </li>
                );
              })}
            </ul>
          ...
          <div className="mt-6">
            <h3 className="text-md font-semibold text-slate-800 dark:text-slate-100 mb-2">Nährstoffwerte (Live)</h3>
            <NutrientBarChart
          ...
          <div className="flex justify-between">
            <Button onClick={() => setStep(1)} variant="secondary">Zurück</Button>
            <Button onClick={() => setStep(3)} variant="primary" disabled={selectedFertilizers.length === 0}>Weiter</Button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div>
            ...
            <ol className="list-decimal list-inside space-y-1">
              <li>Fülle <span className="font-bold">{waterAmount} L</span> Wasser ({WATER_TYPES[waterType]?.name || waterType}) in einen sauberen Behälter.</li>
              {mixSummary.map((line, idx) => (
                <li key={idx}>Füge <span className="font-bold">{line}</span> hinzu und gut umrühren.</li>
              ))}
              <li>Überprüfe den pH- und EC-Wert und passe ggf. an.</li>
            </ol>
            <div className="text-xs text-slate-500 mt-2">Hinweis: Reihenfolge kann je nach Dünger variieren. Immer Herstellerhinweise beachten.</div>
          </div>
          <div className="flex gap-2">
            <Button onClick={() => setStep(2)} variant="secondary">Zurück</Button>
            <Button onClick={handleExport} variant="primary">Mix als JSON exportieren</Button>
          </div>
        </div>
      )}
    </div>
