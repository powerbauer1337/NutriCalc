...
export function calculateNutrientResults({
  ...
  let tN: Record<string, number> = NUTRIENT_FIELDS.reduce((acc: Record<string, number>, f) => { 
    acc[f.key] = 0; 
    return acc; 
  }, {});
  ...
  (selectedFertilizers || []).forEach(fert => {
    ...
    Object.keys(fD.composition).forEach(nk => {
      ...
      let nVppm = (fD.type === 'powder') 
        ? (fA * nC * 10) / waterVolume 
        : (fA * nC) / waterVolume;
